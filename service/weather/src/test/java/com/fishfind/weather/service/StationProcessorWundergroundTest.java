...
class StationProcessorWundergroundTest {
    ...
    @InjectMocks
    private StationProcessorWunderground processor;

    @Test
    void fetchesAndPersistsOnSuccess() throws Exception {
        when(fetcher.fetchCurrent(40.77, -73.98)).thenReturn("{\"observations\":[]}");

        assertThat(processor.process(station)).isEqualTo(ProcessingOutcome.PROCESSED);

        verify(weatherDataRepository).saveStationData("KNYC", "{\"observations\":[]}", WeatherSourceType.WUNDERGROUND_OBSERVATIONS);
    }

    @Test
    void skipsPersistWhenNoNearbyStation() throws Exception {
        when(fetcher.fetchCurrent(40.77, -73.98)).thenThrow(new FileNotFoundException("no nearby station"));
    ...
    void exposesCountryAndMissingSourceMetadata() {
        ...
        assertThat(processor.missingSourceDescription()).isEqualTo("Wunderground source");
