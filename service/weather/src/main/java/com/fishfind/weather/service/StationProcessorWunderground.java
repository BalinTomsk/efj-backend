 ...
 */
@Service
public class StationProcessorWunderground extends StationProcessorBase {
    private static final Logger log = LoggerFactory.getLogger(StationProcessorWunderground.class);

    private final WundergroundFetcher fetcher;
    private final WeatherDataRepository weatherDataRepository;

    public StationProcessorWunderground(WundergroundFetcher fetcher,
    ...
    protected void verifyStation(StationRef station) throws Exception {
        String json = fetcher.fetchCurrent(station.latitude(), station.longitude());
        log.info("Startup Wunderground verification fetched payload. station={} state={} bytes={}",
                station.mli(), station.state(), json.length());
    }
