...
/**
 * Fetches raw current-conditions JSON from Google Maps Platform Weather API.
 */
@Service
public class GoogleWeatherFetcher {
    private static final Logger log = LoggerFactory.getLogger(GoogleWeatherFetcher.class);
    ...
    @Value("${weather.worker.google-weather-base-url:https://weather.googleapis.com/v1/currentConditions:lookup}")
    ...
    @Retry(name = "googleWeather")
    @CircuitBreaker(name = "googleWeather")
    @RateLimiter(name = "googleWeather")
        ...
        while (true) {
            ...
            try {
                ...
                if (status == HttpURLConnection.HTTP_NOT_FOUND) {
                    throw new FileNotFoundException("Google Weather feed not published for latitude="
                            + latitude + " longitude=" + longitude);
                }
                if (status == HttpURLConnection.HTTP_UNAUTHORIZED || status == HttpURLConnection.HTTP_FORBIDDEN) {
                    throw new IOException("Google Weather authentication failed with HTTP " + status);
                }
                if (status == HTTP_TOO_MANY_REQUESTS) {
                    if (rateLimitWaits >= rateLimitMaxRetries) {
                        throw new RateLimitedException(
                                "Google Weather rate limited (429) after " + rateLimitWaits + " waits");
                    }
                    long waitMs = retryAfterMillis(connection);
                    rateLimitWaits++;
                    log.warn("Google Weather rate limited (429). Honouring Retry-After. waitMs={} attempt={}",
                            waitMs, rateLimitWaits);
                    honourRetryAfter(waitMs);
                    continue;
                }
                if (status != HttpURLConnection.HTTP_OK) {
                    throw new IOException("Google Weather returned HTTP " + status);
                }

                try (InputStream inputStream = connection.getInputStream()) {
                    byte[] body = inputStream.readNBytes(maxResponseBytes + 1);
                    if (body.length > maxResponseBytes) {
                        throw new IOException("Google Weather response exceeded " + maxResponseBytes + " bytes");
                    }
                    String json = new String(body, StandardCharsets.UTF_8);
                    requireJsonObjectShape(json);
                    log.debug("Google Weather fetch succeeded. latitude={} longitude={}", latitude, longitude);
                    return json;
                }
            } finally {
    ...
    private void honourRetryAfter(long ms) throws IOException {
        try {
            ...
            throw new RateLimitedException("Interrupted while waiting out Google Weather Retry-After", ex);
        }
    }
    ...
    private static void requireJsonObjectShape(String body) throws IOException {
        ...
        if (trimmed.isEmpty() || trimmed.charAt(0) != '{') {
            throw new IOException("Google Weather returned a non-JSON body");
        }
    }
