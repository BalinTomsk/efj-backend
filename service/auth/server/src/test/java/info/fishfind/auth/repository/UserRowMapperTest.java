...
class UserRowMapperTest {
    ...
    void mapRowMapsAllColumnsToUser() throws SQLException {
        ...
        when(resultSet.getString("ip4")).thenReturn("127.0.0.1");
        when(resultSet.getString("ip6")).thenReturn("");
        when(resultSet.getString("titul")).thenReturn("Captain");
        when(resultSet.getObject("last_visit", OffsetDateTime.class)).thenReturn(OffsetDateTime.parse("2026-03-12T03:00:00Z"));
        when(resultSet.getString("question")).thenReturn("River?");
        when(resultSet.getString("answer")).thenReturn("Salmon");
        when(resultSet.getString("cell")).thenReturn("555-0100");
        when(resultSet.getBoolean("suspended")).thenReturn(false);
        when(resultSet.getString("agent")).thenReturn("JUnit");
        ...
        assertThat(user.ip4()).isEqualTo("127.0.0.1");
        assertThat(user.ip6()).isEmpty();
        assertThat(user.titul()).isEqualTo("Captain");
        assertThat(user.lastVisit()).isEqualTo(OffsetDateTime.parse("2026-03-12T03:00:00Z"));
        assertThat(user.question()).isEqualTo("River?");
        assertThat(user.answer()).isEqualTo("Salmon");
        assertThat(user.cell()).isEqualTo("555-0100");
        assertThat(user.suspended()).isFalse();
        assertThat(user.agent()).isEqualTo("JUnit");
