...
`dbo.fn_lake_view_json` / `dbo.fn_lake_fishing_json` / `dbo.sp_lake_fish_upsert_batch`, all existing in
`envfish-db`; see [Data access](#data-access)):
...
`mvn test` runs 105 tests, none requiring a database:
