 ...
 *
 * <p>Credentials are never copied into JVM-global system properties via {@code System.setProperty}
 * (which would leak secrets into {@code System.getProperties()}, heap dumps, and diagnostic tooling).
 * Because the property source is registered with {@code addLast}, real OS environment variables and
 * JVM system properties always take precedence, so production deployments that inject
 * {@code DB_URL}/{@code DB_USERNAME}/{@code DB_PASSWORD} as environment variables are unaffected and
 * the {@code .env} file is only a local fallback.
 */
public class DotenvEnvironmentPostProcessor implements EnvironmentPostProcessor {
     ...
     *
     * <p>This is the path that matters in production: docapi is deployed with Docker's
     * {@code --env-file} (see {@code docs/install-jnode.md} — the {@code DOTENV_PATH} mount the workers
     * use cannot be used here, as the datasource then receives the literal {@code ${DB_URL}}), which is
     * read by the daemon and injects the values as genuine environment variables, so the file-loading
     * path above never sees them. The decrypted values are added directly <em>before</em> the
     * {@code systemEnvironment} source rather than with {@code addFirst}, so command-line arguments and
     * any test override still take precedence.
     *
